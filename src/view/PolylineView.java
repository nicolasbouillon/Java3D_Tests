...
public class PolylineView {
	...
	public Shape3D display() {
		...
		for (int i = 1; i < count - 1; i++) {
			triangles1[i - 1] = new TriangleArray(3, TriangleArray.COORDINATES
					| TriangleArray.COLOR_3);
			triangles2[i - 1] = new TriangleArray(3, TriangleArray.COORDINATES
					| TriangleArray.COLOR_3);

			Point3d pointa = new Point3d(this.polyline.getPointList().get(0)
					.getX(), this.polyline.getPointList().get(0).getY(),
					this.polyline.getPointList().get(0).getZ());
			Point3d pointb = new Point3d(this.polyline.getPointList()
					.get((i) % count).getX(), this.polyline.getPointList()
					.get((i) % count).getY(), this.polyline.getPointList()
					.get((i) % count).getZ());
			Point3d pointc = new Point3d(this.polyline.getPointList()
					.get((i + 1) % count).getX(), this.polyline.getPointList()
					.get((i + 1) % count).getY(), this.polyline.getPointList()
					.get((i + 1) % count).getZ());
